...
export function SMSDrawer({ appointment, onClose, onMarkComplete, onScheduleFollowUp, readOnly = false }: SMSDrawerProps) {
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 z-50 flex h-full w-full max-w-md flex-col border-l border-border bg-surface shadow-2xl transition-transform">
              ...
              <div key={msg.id} className={`flex ${msg.from === 'system' ? 'justify-start' : 'justify-end'}`}>
                <div
                  className={`max-w-[80%] rounded-2xl px-3.5 py-2.5 text-sm shadow-sm ${
                    msg.from === 'customer'
                      ? 'rounded-tr-sm border border-border bg-surface-elevated text-text-primary'
